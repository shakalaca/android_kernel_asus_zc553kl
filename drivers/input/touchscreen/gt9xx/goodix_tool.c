...
static int get_current_time_string(char *buf)
{	    
	struct timex  txc;
	struct rtc_time tm;
...
static void tool_set_proc_name(char * procname)
{
   ...
   // sprintf(date, "%s", __DATE__);
    ...
    if( get_current_time_string(date)!=8 ){
	GTP_DEBUG("get_current_time_string failed\n");
	sprintf(procname, "gmnode00000000"); 
	return;
    }
    sprintf(procname, "gmnode%s",date); 
