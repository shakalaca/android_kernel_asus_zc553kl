...
#endif
//Start:add hardware info for factory mode ByElan 20160628
#include <linux/hardware_info.h> 
//End:add hardware info for factory mode ByElan 20160628
...
static int panel_x_left = 0, panel_x_right = 0;
#endif
...
#if CTP_HAVE_KEYS
			if(event->au16_y[i] > panel_y)
			{
				key_flag=1;
				if (event->au16_x[i] <= panel_x_left)
				{
					key_value = KEY_BACK;
					input_report_key(data->input_dev, KEY_BACK, 1);
				}
				else if(event->au16_x[i] >= panel_x_right)
				{
					key_value = KEY_MENU ;
...
static int fts_gpio_configure(struct fts_ts_data *data, bool on)
{
	...
	if (on) {
		...
		if (gpio_is_valid(data->pdata->reset_gpio)) {
			...
			//-----Start Set VDD RESET up time sequence at 10ms ByElan 20160627------
			//gpio_set_value_cansleep(data->pdata->reset_gpio, 1);
			//-----End Set VDD RESET up time sequence at 10ms ByElan 20160627------
		}
...
static int fts_power_on(struct fts_ts_data *data, bool on)
{
	...
	msleep(10);	// xieh add delay between power_on & reset: spec > 10ms 0623
...
power_off:
	...
	msleep(4); // xieh add delay after power_down 0623
...
//End:Reqxxx,liuyang3,wt,Modify,for add gesture switch.
	...
	#endif
        //Start:Bug197978 TP休眠漏电 Modify: TP使用指令休眠，取消下电 20160713 ByElan
	data->suspended = true;
	fts_write_reg(fts_i2c_client, 0xa5, 0x03); //enter minitor mode.
	return 0;
        //End::Bug197978 TP休眠漏电 Modify: TP使用指令休眠，取消下电 
}
...
#define FTS_VIRTUAL_KEY_ARR_SIZE	3
static int fts_get_dt_virtual_key(struct device *dev, char *name, struct fts_ts_platform_data *pdata)
{
	u32 virtual_key[FTS_VIRTUAL_KEY_ARR_SIZE];
	struct property *prop;
	struct device_node *np = dev->of_node;
	int virtual_key_size, rc;
	...
	virtual_key_size = prop->length / sizeof(u32);
	if (virtual_key_size != FTS_VIRTUAL_KEY_ARR_SIZE) {
		dev_err(dev, "invalid %s\n", name);
		return -EINVAL;
	}

	rc = of_property_read_u32_array(np, name, virtual_key, virtual_key_size);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read %s\n", name);
		return rc;
	}

	if (!strcmp(name, "focaltech,virtual-key1")) {
		pdata->virtual_key1_x = virtual_key[1];
		pdata->virtual_key1_y = virtual_key[2];
#if CTP_HAVE_KEYS
        panel_x_right = pdata->virtual_key1_x;
#endif
	} else if (!strcmp(name, "focaltech,virtual-key2")) {
		pdata->virtual_key2_x = virtual_key[1];
		pdata->virtual_key2_y = virtual_key[2];
	} else if (!strcmp(name, "focaltech,virtual-key3")) {
		pdata->virtual_key3_x = virtual_key[1];
		pdata->virtual_key3_y = virtual_key[2];
#if CTP_HAVE_KEYS
        panel_x_left = pdata->virtual_key3_x;
#endif
	} else {
		dev_err(dev, "unsupported property %s\n", name);
		return -EINVAL;
...
#if CTP_HAVE_KEYS
    pr_info("%s: x_max=%d, y_max=%d\n", __func__, pdata->x_max, pdata->y_max);
    panel_x = pdata->x_max;
    panel_y = pdata->y_max;
#endif

	rc = fts_get_dt_virtual_key(dev, "focaltech,virtual-key1", pdata);
	if (rc)
		dev_err(dev, "Unable to get virtual-key1 \n");
	rc = fts_get_dt_virtual_key(dev, "focaltech,virtual-key2", pdata);
	if (rc)
		dev_err(dev, "Unable to get virtual-key2 \n");
	rc = fts_get_dt_virtual_key(dev, "focaltech,virtual-key3", pdata);
	if (rc)
		dev_err(dev, "Unable to get virtual-key3 \n");
...
//End:Reqxxx,liuyang3.wt,Modify,20160512, for report KEY event.	
	...
	//Start Trtp Trsi  byElan 160627
	msleep(5);
	gpio_set_value_cansleep(data->pdata->reset_gpio, 1);
	msleep(200);
	//End Trtp Trsi  byElan 160627
...
//End:Reqxxx,liuyang3.wt,ADD,20160521, for glove mode switch.	
	...
	//Start:add hardware info ByElan 20160628
#if 1
	err = hardwareinfo_set_prop(HARDWARE_TP,"ft5436_V3");
    if (err < 0){
	    pr_err("[TP] Failed to hardwareinfo_set_prop, %d\n",err);
		//return -1;
    }else{
		//printk("[TP] fts_ts_probe hardwareinfo_set_prop HARDWARE_TP ft5436_V3\n");
	}
#endif	
	//End:add hardware info ByElan 20160628
