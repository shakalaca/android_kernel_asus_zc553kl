...
printk(KERN_ERR"   gf_parse_dts ========.\n");
 ...
 //   rc = gpio_request(gf_dev->irq_gpio, "gf_irq-gpio");
    if (gpio_is_valid(gf_dev->irq_gpio)){
		...
		printk(KERN_ERR"not valid irq gpio\n");
		return -EIO;
   ...
   // gpio_direction_input(gf_dev->irq_gpio);
	...
	if (gpio_is_valid(gf_dev->reset_gpio)){
	    ...
	    if(rc){
	          printk(KERN_ERR"could not request reset gpio\n");
	          return rc;
	    }
	}
	else{
		printk(KERN_ERR"not valid reset gpio\n");
		return -EIO;
	}
...
int driver_init_partial(struct gf_dev *gf_dev)
{
...
void gf_power_down(struct gf_dev* gf_dev)
{
