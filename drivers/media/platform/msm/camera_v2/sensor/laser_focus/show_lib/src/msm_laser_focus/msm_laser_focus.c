...
int Verify_ID_is_Correct(struct msm_laser_focus_ctrl_t *dev_t, int chip_id_size){
	...
	// Verify ID
	LOG_Handler(LOG_ERR,"%s: read id: 0x%x expected id 0x%x\n", __func__, chip_id, slave_info->sensor_id);
...
#else
		rc = gpio_direction_output(dev_t->pwr_en_gpio, 1);
#endif
		if (rc < 0) {
...
#else
		rc = gpio_direction_output(dev_t->pwr_en_gpio, 0);
#endif
		if (rc < 0) {
			LOG_Handler(LOG_ERR, "%s failed %d\n", __func__, __LINE__);
			return rc;
...
static const struct file_operations user_version_debug1 = {
	.owner = THIS_MODULE,
	.read = seq_read,
	.llseek = seq_lseek,
...
int dev_I2C_status_check(struct msm_laser_focus_ctrl_t *dev_t, int chip_id_size){
	...
	{
		LOG_Handler(LOG_CDBG, "%s: start to try read id\n", __func__);
	do{
		rc =power_up(dev_t);
				if (rc < 0) {
		...
		LOG_Handler(LOG_CDBG, "%s: power success, wait 100ms then to read id\n", __func__);
		msleep(100);//wait device power up success.
		rc = Laser_Match_ID(dev_t, chip_id_size);
		if (rc < 0) {
			sprintf(debug_string,"driver/power_fail%d",time_index);
			proc(debug_string, 0664, &user_version_debug1);
			LOG_Handler(LOG_ERR, "%s: dev_match_id failed %d, deinit the device and powndown the power and try again\n", __func__, __LINE__);
			...
			msleep(100);
			time_index--;
		    LOG_Handler(LOG_CDBG, "%s: read id failed, current try time is %d\n", __func__,time_index);
			if(time_index<0)
			{
			break; //after a time , if the device can not comunication, break the loop, but not return erro, 
		    LOG_Handler(LOG_CDBG, "%s: read id failed, try times have reach max, break the trying\n", __func__);
			}//because after the device bring up,the communication will be ok in usr version.
		}
	}while(rc<0);

	    
		LOG_Handler(LOG_CDBG, "%s: here the read id success or faild rc=%d\n", __func__,rc);
		Laser_Match_Module(dev_t);

		/* Deinitialize device */

		LOG_Handler(LOG_CDBG, "%s: we have completed the read id ,power down the devices%d\n", __func__,rc);
		rc = dev_deinit(dev_t);
		if (rc < 0) {
	...
	{
		LOG_Handler(LOG_CDBG, "%s:current device stats not to need match id\n", __func__);
		Laser_Match_Module(dev_t);
	}


	LOG_Handler(LOG_CDBG, "%s: Exit always return match success, because some device hardware issue\n", __func__);
