...
int	Olivia_Do_TOF_and_Calibration(struct msm_laser_focus_ctrl_t *dev_t, enum laura_configuration_ctrl  cal_type, uint16_t* cal_data){
	...
	if(status < 0)
	{
		LOG_Handler(LOG_ERR,"czw, Olivia_device_tof_configuration_interface return error %d\n",status);
		return status;
	}
	...
	if(status < 0)
	{
		LOG_Handler(LOG_ERR,"czw, Laura_device_calibration return error %d\n",status);
		return status;
	}
...
int Laura_device_read_range_interface(struct msm_laser_focus_ctrl_t *dev_t, bool load_cal, bool *calibration_flag){
	...
	status = Laura_device_wake_up_interface(dev_t, load_cal, calibration_flag, DO_MEASURE);
	if(status < 0){
		LOG_Handler(LOG_ERR,"czw, Laura_device_wake_up_interface failed, return %d\n",status);
		Laura_MCPU_Controller(dev_t, MCPU_STANDBY);
		return status;
	}
#if 1 //just do TOF configuration for calibration
	/* Set TOF configuration */
	status = Olivia_device_tof_configuration_interface(dev_t, MSM_LASER_FOCUS_APPLY_NORMAL_CALIBRATION);
	if(status < 0){
		LOG_Handler(LOG_ERR,"czw, Olivia_device_tof_configuration_interface failed, return %d\n",status);
		Laura_MCPU_Controller(dev_t, MCPU_STANDBY);
		return status;
	}
#endif
	...
	if(status < 0){
		LOG_Handler(LOG_ERR,"czw, Olivia_device_read_range failed, return %d\n",status);
		Laura_MCPU_Controller(dev_t, MCPU_STANDBY);
		return status;
	}
...
int Laura_device_wake_up_interface(struct msm_laser_focus_ctrl_t *dev_t, bool load_cal, bool *cal_flag, bool do_measure){
	...
	if(status ==0)
		...
		LOG_Handler(LOG_ERR,"WaitMCPUOn time out, not do ambient_setting\n");
	/* Check if do measure */
	if(!do_measure){
		/* Go MCPU to standby mode */
		status = Laura_non_measures_go_standby(dev_t);
