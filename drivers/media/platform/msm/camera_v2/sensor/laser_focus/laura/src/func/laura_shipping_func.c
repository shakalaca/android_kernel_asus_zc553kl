...
int Olivia_Mailbox_Command(struct msm_laser_focus_ctrl_t *dev_t, uint16_t cal_data[]){
	...
	if(status<0)
	{
		LOG_Handler(LOG_ERR,"czw, Check_Ready_via_ICSR return error %d\n",status);
		return status;
	}
	//if(status !=0)
...
int Olivia_device_Load_Calibration_Value(struct msm_laser_focus_ctrl_t *dev_t){
    ...
    uint16_t reg_signal_control0, reg_signal_control1;
    uint16_t reg_signal_control[2];
	static uint16_t data[SIZE_OF_OLIVIA_CALIBRATION_DATA+CONFIDENCE_LENGTH];
    static uint16_t signal_control1[] = {0x1AD6, 0x86D5};
    static uint16_t signal_control2[] = {0x64D5, 0x00E5};
    static uint16_t signal_control3[] = {0x0300};
    ...
    Laura_device_indirect_addr_write(dev_t, 0x18, 0x19, 0x1007, I2C_DATA_PORT, signal_control1, 2);
    Laura_device_indirect_addr_write(dev_t, 0x18, 0x19, 0x3007, I2C_DATA_PORT, signal_control2, 2);
    Laura_device_indirect_addr_write(dev_t, 0x18, 0x19, 0x0007, I2C_DATA_PORT, signal_control3, 1);

    /* Check signal control write */
    CCI_I2C_WrByte(dev_t, 0x18, 0x10);
    CCI_I2C_WrByte(dev_t, 0x19, 0x07);
    for(i=0; i<2; i++){
       CCI_I2C_RdByte(dev_t, I2C_DATA_PORT, &reg_signal_control0);
       CCI_I2C_RdByte(dev_t, I2C_DATA_PORT, &reg_signal_control1);
       reg_signal_control0 &= 0x00ff;
       reg_signal_control1 &= 0x00ff;
       reg_signal_control[i] = (reg_signal_control1<<8)|reg_signal_control0;
    }
    LOG_Handler(LOG_CDBG, "%s: Signal Control-0x0710: 0x%04x 0x%04x \n",__func__, reg_signal_control[0],reg_signal_control[1]);
    ...
    LOG_Handler(LOG_CDBG, "%s: Signal Control-0x0730: 0x%04x 0x%04x \n",__func__, reg_signal_control[0],reg_signal_control[1]);
    ...
    reg_signal_control0 &= 0x00ff;
    reg_signal_control1 &= 0x00ff;
    reg_signal_control[0] = (reg_signal_control1<<8)|reg_signal_control0;
    LOG_Handler(LOG_CDBG, "%s: Signal Control-0x0700: 0x%04x \n",__func__, reg_signal_control[0]);
...
int Olivia_Read_Calibration_Value_From_File(struct seq_file *vfile, uint16_t *cal_data){
	...
	if((output_len=format_hex_string(log_buf,sizeof(log_buf),cal_data,SIZE_OF_OLIVIA_CALIBRATION_DATA)) != -1)
	{
		LOG_Handler(LOG_CDBG,"Cal data (log buf len %d)\n%s",output_len,log_buf);
	}
	else
	{
	...
	if(RawRange&VALID_DATA){
		...
		if(!result)
		{
			...
			else
			{
				...
				else if(error_status==NEAR_FIELD && Range == 0 && confidence_level == 0){
                	if(IT_verify < ItB) {
                		result = 6;
                		Range = 0;//IN_RANGE ?
                		errcode = RANGE_ADAPT;
				...
				else if(error_status==FAR_FIELD && Range==0 && confidence_level == 0){
                	if(IT_verify < ItB) {
                		result = 71;
                		Range =	OUT_OF_RANGE;
                		errcode = RANGE_ERR_NOT_ADAPT;
...
int getTOF(uint16_t* config_normal,uint16_t* config_K10,uint16_t* config_K40,int LAURA_CONFIG_SIZE){
	...
	status = Sysfs_read_word_seq("/persist/Olivia_conf.txt",buf,3*LAURA_CONFIG_SIZE);
	if(status<0)
		return status;
...
int Laura_WaitDeviceStandby(struct msm_laser_focus_ctrl_t *dev_t){
	...
	while(1){
		...
		now = get_current_time();
				if(is_timeout(start,now,TIMEOUT_VAL)){
			LOG_Handler(LOG_ERR, "%s: Wait chip standby time out - register(0x06): 0x%x, state %s\n", __func__, i2c_read_data,getMCPUState(i2c_read_data));
				status = -TIMEOUT_VAL;
			break;
				}
#endif
...
static void getRegisterData(struct msm_laser_focus_ctrl_t *dev_t,uint16_t address, uint16_t *data, uint32_t data_len)
{
	...
	{
		//one word
		CCI_I2C_RdByte(dev_t, 0x1A, &data[i]);
    }
}
